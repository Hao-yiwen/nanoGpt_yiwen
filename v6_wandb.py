# -*- coding: utf-8 -*-
"""
NanoGPT v6 - 中文歌词版本 (Wandb 日志版)

基于 v5_checkpoint.py，新增 wandb 日志记录：
...
WARMUP_ITERS = 300       # 预热迭代次数（3000步的10%）
TRAIN_ITERS = 3000       # 训练迭代次数
...
# v6 新增：Wandb 配置
WANDB_PROJECT = 'nanogpt-lyrics'
WANDB_RUN_NAME = None    # None 则自动生成
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    context = torch.tensor([[sp.bos_id()]], dtype=torch.long, device=DEVICE)
    final_generated = decode(model.generate(context, max_new_tokens=FINAL_GENERATE_TOKENS)[0].tolist())
    print(final_generated)

    # v6: 记录最终生成到 wandb
    if USE_WANDB:
        wandb.log({'final_generated_text': wandb.Html(f'<pre>{final_generated}</pre>')})
        wandb.finish()
        print("\nWandb 日志已完成")
